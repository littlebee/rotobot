...
def rotate_360(times=1):
    # avoid jerking to full speed
    log.info("accelerating motor")
    motor.throttle = -0.5
    time.sleep(0.25)
    motor.throttle = -0.6
    time.sleep(0.25)
    ...
    log.info(f"waiting for {times} rotations")
    for i in range(0, times):
        ...
        log.info(f"waiting for switch to be triggered on {SWITCH_PIN}")
